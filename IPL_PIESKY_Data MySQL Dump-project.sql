...
select  i1.BIDDER_ID,i2.NO_OF_BIDS ,count(i1.bidder_id) as NUMBER_OF_WINS,
(count(i1.bidder_id)/i2.NO_OF_BIDS)*100  AS PERCENTAGE_WIN 
from  ipl_bidding_details  as  i1
join    ipl_bidder_points  as  i2
on  i1.bidder_id=i2.bidder_id
where  i1.BID_STATUS = 'Won'
group by i1.bidder_id
order by (count(i1.bidder_id)/i2.NO_OF_BIDS)*100   DESC ;

#HERE TWO TABLES  IPL_BIDDING_DETAILS  AND  IPL_BIDDER_POINTS ARE USED....
...
select  c.stadium_id,count(d.schedule_id) as number_of_matches_at_each_stadium,c.stadium_name,c.city,c.address 
 from   ipl_stadium  as c
join  ipl_match_schedule as d
on  c.stadium_id=d.stadium_id
group by c.stadium_id
order by count(d.schedule_id);
#HERE WE USE TWO  TABLES IPL_STADIUM  AND IPL_MATCH_SCHEDULE
...
select b.stadium_name,b.stadium_id,a.match_id,a.status,
count(a.stadium_id)  as  no_of_matches,
sum(if(c.toss_winner=c.match_winner,1,0)) as no_of_teams_win_toss_and_match,
(sum(if(c.toss_winner=c.match_winner,1,0))/count(b.stadium_id))*100  as percentage_of_wins_by_a_team_who_won_toss
 from ipl_match_schedule as a
join ipl_stadium as b
...
select b1.bid_team,b2.TEAM_NAME,count(b1.bidder_id) as Total_bids from  ipl_bidding_details  as b1
join  ipl_team as b2
on   b1.bid_team=b2.team_id
group by b1.bid_team
order by b1.bid_team;
...
select a.team_id,a.team_name,b.team_id1,b.team_id2,b.match_winner,b.win_details 
 from ipl_team as a
join  ipl_match as b
on  substr(a.remarks,1,3) =substr(b.win_details,6,3);    #AS PER THE WIN DETAILS
...
select a.team_id,b.TEAM_NAME,sum(a.MATCHES_WON)  AS total_matches_won,sum(a.MATCHES_LOST)  AS Total_matches_lost,
sum(a.MATCHES_TIED) AS total_matches_tied ,sum(a.NO_RESULT) AS total_matches_noresult,
...
#Here we are using  two  tables  IPL_TEAM AND IPL_TEAM_STANDINGS
...
select a.PLAYER_ID,a.PLAYER_NAME,b.TEAM_ID,b.PLAYER_ROLE,b.REMARKS,c.TEAM_NAME from  ipl_player  as a
join  ipl_team_players as b
on a.PLAYER_ID=b.PLAYER_ID
join  ipl_team as c
on  b.team_id=c.team_id
where c.TEAM_NAME like '%Mumbai%' AND b.PLAYER_ROLE LIKE '%Bowler%'
order by a.PLAYER_ID;



#HERE WE ARE USING THREE TABLES  IPL_TEAM_PLAYER,IPL_TEAM,IPL_PLAYER 
...
#METHOD1
select b1.TEAM_NAME,a1.PLAYER_ROLE,count(a1.PLAYER_ROLE) from  ipl_team_players as a1
join  ipl_team as b1
on a1.team_id=b1.team_id
join ipl_player as  c1
on  a1.player_id=c1.player_id
group by  a1.PLAYER_ROLE,b1.TEAM_NAME
having a1.PLAYER_ROLE LIKE '%All-Rounder%' AND count(a1.PLAYER_ROLE)>4
ORDER BY count(a1.PLAYER_ROLE) desc;
